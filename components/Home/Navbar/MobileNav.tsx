...
const MobileNav = ({ showNav, closeNav }: MobileNavProps) => {
  ...
  return (
    <div>
      ...
      <div className={`text-white fixed ${navOpen} justify-center flex flex-col h-full transform transition-all duration-500 delay-350 w-[80%] sm:w-[60%] bg-cyan-800 space-y-6 z-[1000050] right-0`}>
        {NavLinks.map((link) => {
          return (
        ...
        <CgClose
          onClick={closeNav}
          className="absolute top-[0.7rem] right-[1.4rem] cursor-pointer sm:w-8 sm:h-8 w-6 h-6"
        />
      </div>
    </div>
  );
